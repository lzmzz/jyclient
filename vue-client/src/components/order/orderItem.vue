<template>
    <div>
        ...
        <cell :title="'订单状态'">
            <div><span style="color: #09BB07">{{statusArr[orderData.order_status]}}</span></div>
        </cell>
        <cell :title="'订单数量'" :value="orderData.order_many"></cell>
        <cell :title="'实际完成数量'">
            <input type="number" class="orderInput" placeholder="请输入" v-model="orderData.status_many" v-if="!orderData.status_exist"/>
            <div class="orderInput" v-else>{{orderData.status_many}}</div>
        </cell>
        <Button round type="primary" v-if="!orderData.status_exist" @click.native="addStatusDtl" class="qdBtn">确定</Button>
        <Button round type="danger" v-if="userInfo.is_master==0" @click.native="setOrderStatus" class="qdBtn">已完成</Button>
    </div>
</template>
        ...
        methods: {
            setOrderStatus() {
                ...
                this.$dialog.confirm({
                    ...
                    that.$http.post('/jyclient/api/order/setOrderStatus', params).then(res=>{
                        if(res.status==0){
                            this.$toast(res.data)
                            setTimeout(() => {
                                that.$router.push({
            ...
            getOrderItem() {
                ...
                this.$http.post('/jyclient/api/order/getOrderItem', params).then(res=>{
                    if(res.status==-1){
                        this.$toast(res.data)
                    }else{
...
<style scoped>
.orderInput{display: block;height: .5rem;width: 100%;text-align: right;font-size: 0.56rem;color: #f5871f;}
.qdBtn{width: 80%;margin: 1rem auto;display: block;}
</style>